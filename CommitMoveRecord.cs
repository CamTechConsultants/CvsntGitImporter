		...
		public void Apply(IList<Commit> commitStream)
		{
			...
			for (int i = m_commits.Count - 1; i >= 0; i--)
			{
				...
				if (fileRevisions.Count() < commitToMove.Count())
				{
					...
					var commit1 = new Commit(commitToMove.CommitId + "-1");
					m_log.WriteLine("New commit {0}", commit1.CommitId);
					m_log.Indent();
					foreach (var f in commitToMove.Except(fileRevisions))
					{
						m_log.WriteLine("{0}", f);
						commit1.Add(f);
					}
					...
					var commit2 = new Commit(commitToMove.CommitId + "-2");
					m_log.WriteLine("New commit {0}", commit2.CommitId);
					m_log.Indent();
					foreach (var f in fileRevisions)
					{
						m_log.WriteLine("{0}", f);
						commit2.Add(f);
					}
